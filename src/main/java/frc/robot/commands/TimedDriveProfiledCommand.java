  ...
  public TimedDriveProfiledCommand(double xSpeed, double ySpeed, double targetAngle) {
    super(
        ...
        new TrapezoidProfile(
            ...
            new TrapezoidProfile.Constraints(DrivetrainSubsystem.MAX_VELOCITY_METERS_PER_SECOND / 5, DrivetrainSubsystem.MAX_VELOCITY_METERS_PER_SECOND / 2),
            // Goal state
            new TrapezoidProfile.State(),
            // Initial state
            new TrapezoidProfile.State()),
        state -> {
          double x = state.velocity;
          double y = state.velocity;
