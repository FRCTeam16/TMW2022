...
public class SimpleDistanceDriveCommand extends CommandBase {
  ...
  public void initialize() {
    var currentPose = Subsystems.drivetrainSubsystem.getPose();
    var translation = new Translation2d(xdist, ydist);
    var xy_trans = currentPose.getTranslation().plus(translation);
    this.targetPose = new Pose2d(xy_trans.getX(), xy_trans.getY(), new Rotation2d());
  ...
  public void execute() {
    ...
    var driveAngle = Math.atan2(targetPose.getY() - currentPose.getY(), targetPose.getX() - currentPose.getX());
    ...
    var speeds = ChassisSpeeds.fromFieldRelativeSpeeds(
        vxMetersPerSecond, vyMetersPerSecond, Math.toRadians(twist), Subsystems.drivetrainSubsystem.getGyroscopeRotation());
  ...
  public boolean isFinished() {
    var distance = Subsystems.drivetrainSubsystem.getPose().getTranslation().getDistance(targetPose.getTranslation());
